...
class SWOTRequest(BaseModel):
    ...
    business_description: str = Field(
        ..., min_length=10, description="Description of the business to analyze"
    )
    company_name: Optional[str] = Field(
        default="", description="Optional company name"
    )
        ...
        json_schema_extra = {
            "example": {
                "business_description": (
                    "A local coffee shop in Jakarta that serves "
...
class SWOTResponse(BaseModel):
    ...
    strengths: List[str] = Field(
        ..., description="List of business strengths"
    )
    weaknesses: List[str] = Field(
        ..., description="List of business weaknesses"
    )
    ...
    company_name: Optional[str] = Field(
        default="", description="Company name if provided"
    )
