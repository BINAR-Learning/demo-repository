...
async function sendMetric(baseUrl, metricName, value, labels = {}) {
  try {
    const response = await axios.post(`${baseUrl}/api/metrics/record`, {
      metric: metricName,
      value: value,
      labels: labels
    }, {
...
async function testEndpointAndRecordMetrics(baseUrl, endpoint, method = 'GET') {
  ...
  try {
    const response = await axios({
      ...
      timeout: 300000, // 5 minutes for heavy queries
      headers: { 'Content-Type': 'application/json' }
    });
...
async function generateApiMetrics() {
  ...
  for (const endpoint of testConfig.endpoints) {
    console.log(`🎯 Generating metrics for ${endpoint.name} (${endpoint.method} ${endpoint.path})`);
    ...
    for (let batch = 0; batch < Math.ceil(testConfig.totalRequests / testConfig.concurrentRequests); batch++) {
      ...
      for (let i = 0; i < testConfig.concurrentRequests; i++) {
        ...
        if (requestNumber <= testConfig.totalRequests) {
          batchPromises.push(
            testEndpointAndRecordMetrics(baseUrl, endpoint.path, endpoint.method)
              .then(result => {
                totalRequests++;
                
                if (result.success) {
                  endpointSuccess++;
                  console.log(`   ✅ Request ${requestNumber}/${testConfig.totalRequests} - ${result.duration.toFixed(3)}s (Status: ${result.status})`);
                } else {
                  endpointFailed++;
                  console.log(`   ❌ Request ${requestNumber}/${testConfig.totalRequests} - ${result.error} (${result.duration.toFixed(3)}s)`);
                }
    ...
    console.log(`   - Successful Requests: ${endpointSuccess}/${testConfig.totalRequests}`);
    console.log(`   - Failed Requests: ${endpointFailed}/${testConfig.totalRequests}`);
    console.log(`   - Metrics Sent: ${endpointSuccess * 2 + endpointFailed * 2}\n`);
  }
